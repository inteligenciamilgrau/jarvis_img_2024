...
class AnthropicToolHandler:
    ...
    def grab_screen_of_monitor(self):
        with mss.mss() as sct:
            ...
            if self.monitor_index < 1 or self.monitor_index >= len(monitors):
                raise ValueError(f"Índice de monitor inválido: {self.monitor_index}. Monitores disponíveis: 1 a {len(monitors) - 1}")
    ...
    # Função para chamar o método `speak_response` em uma nova thread
    ...
    def handle_tool_response(self, response):
        ...
        if action == "screenshot":
            ...
            self.messages.append(self.template_resposta_tool(f"Tecla {key_text} pressionada", tool_id))
            print("Resposta Key:", response.content[0].text, "\n")
            output_message = f"Tecla {key_text} pressionada."

        elif action == "left_click_drag":
            coord = response.content[1].input['coordinate']
            self.left_click_drag(coord[0], coord[1])
            self.messages.append(self.template_resposta_tool("Clique esquerdo e arrastar realizado", tool_id))
            print("Resposta Left Click Drag:", response.content[0].text, "\n")
            output_message = "Clique esquerdo e arrastar realizado."
            ...
            self.messages.append(self.template_resposta_tool(f"Posição do cursor: x={x}, y={y}", tool_id))
            print("Resposta Cursor Position:", response.content[0].text, "\n")
            output_message = f"Posição do cursor: x={x}, y={y}."
    ...
    def handle_chat(self, question):
        ...
        while response.stop_reason == "tool_use":
            ...
            response = self.perguntando(self.messages)
            print("Resposta da Ferramenta:", response.content[0].text, "\n")
