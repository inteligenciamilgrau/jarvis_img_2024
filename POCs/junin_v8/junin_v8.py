...
class ChatbotApp:
    ...
    def responder(self, resposta):
        if resposta.startswith("Pensamento:"):
                self.display_message(resposta, "lightblue")
        else:
            self.display_message("Bot: " + resposta)
            if self.hear_response_var.get():
                self.speak_response(resposta)
    ...
    def display_message(self, message, bg="light gray"):
        # Enable editing in the Text widget
        self.chat_display.config(state='normal')
        
        # Get the starting position before inserting the new text
        start_index = self.chat_display.index('end-1c')  # Position before inserting the message
        
        # Insert the new message
        self.chat_display.insert(tk.END, message + "\n")
        
        # Get the end position after insertion
        end_index = self.chat_display.index('end-1c')  # Position after inserting the message
        
        # Generate a unique tag name for each message
        tag_name = f"highlight_{start_index.replace('.', '_')}"
        
        # Apply the unique tag to the newly inserted text
        self.chat_display.tag_add(tag_name, start_index, end_index)
        
        # Configure the tag with a background color
        self.chat_display.tag_config(tag_name, background=bg)
